...
class ChannelSimulator:
    def __init__(self, trip_probability = 0.02):
        ...
        self.stat = {'ON': True, 'RUP': False, 'RDW': False, 'OVC': False, 'OVV': False, 'UNV': False, 'MAXV': False, 'TRIP': False, 'OVP': False, 'OVT': False, 'DIS': False, 'KILL': False, 'ILK': False, 'NOCAL': False}
    ...
    def _randomize(self):
        ...
        if self.stat["TRIP"] or self.stat["ILK"]:
            self.stat["ON"] = False
            if self.pdwn == 'KILL':
                self.stat["KILL"] = True # not sure if it behaves like this in KILL mode
                self.vmon = 0
                self.imon = 0
                return

        self.vmon = random.gauss(self._vset, 1./3) # 0.3 V standard deviation
        self.imon = random.gauss(self.vmon / 10e3, self.vmon / 100e3) # (uA) lets say there is a resistance of 10 MOhm and 10% standard deviation
...
class ModuleSimulator:
    def __init__(self, nChannels, trip_probability = 0.05):
        ...
        self.channels = [ChannelSimulator(1-(1-trip_probability)**(1./self.number_of_channels)) for i in range(self.number_of_channels)]
        self.board_alarm_status = {'CH0': False, 'CH1': False, 'CH2': False, 'CH3': False, 'PWFAIL': False, 'OVP': False, 'HVCKFAIL': False}
        self.interlock_status = False
        self.interlock_mode = "CLOSED"
        
        # Start the device reading thread
        self.randomize_thread = threading.Thread(target=self.__continous_randomize, daemon=True).start()
