...
class Module(BaseModule):
    ...
    def number_of_channels(self) -> int:
        ...
        response = _write_command(self._serial, self._logger, command, int)
        if len(response) != 1:
    ...
    def firmware_release(self) -> str:
        ...
        response = _write_command(self._serial, self._logger, command, str)
        if len(response) != 1:
            raise ValueError("Wrong number of values were received, one value expected")
        return response[0]
    ...
    def module_status(self) -> dict:
        ...
        response = _write_command(self._serial, self._logger, command, dict)
        if len(response) != 1:
            raise ValueError("Wrong number of values were received, one value expected")
    ...
    def module_voltage_limit(self) -> float:
        ...
        response = _write_command(self._serial, self._logger, command, float)
        if len(response) != 1:
            raise ValueError("Wrong number of values were received, one value expected")
    ...
    def module_supply_voltage(self) -> List[float]:
        ...
        response = _write_command(self._serial, self._logger, command, List[float])
        if len(response) != 7:
            raise ValueError("Wrong number of values were received, one value expected")
    ...
    def configuration_mode(self) -> bool:
        ...
        response = _write_command(self._serial, self._logger, command, bool)
        if len(response) != 1:
            raise ValueError("Wrong number of values were received, one value expected")
    ...
    def serial_baud_rate(self, baud_rate: int) -> None:
        ...
        response = _write_command(self._serial, self._logger, command, None)
    ...
    def enter_configuration_mode(self, serial_number: int):
        ...
        _write_command(self._serial, self._logger, command, None)
